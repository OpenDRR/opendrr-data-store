-- update BldgExpRef_CA_master_v3p2.sql with new daypop values for 3 census divisions
...
-- import daypop_updates.csv to be used for updating exposure table
DROP TABLE IF EXISTS exposure.daypop_updates CASCADE;

-- create table
CREATE TABLE exposure.daypop_updates(
bldgid varchar,
newpopday float,
cduid float
);
...
-- import exposure from csv
COPY exposure.daypop_updates (bldgid,newpopday,cduid)
    FROM 'D:\Workspace\data\source datasets\exposure\daypop_updates.csv'
        WITH 
          DELIMITER AS ','
          CSV HEADER ;


-- update exposure table with new daypop values
UPDATE exposure.canada_exposure a
SET day = b.newpopday
FROM exposure.daypop_updates b
WHERE a.id = b.bldgid;
