...
DROP TABLE IF EXISTS gmf.shakemap_acm7p2_lrdmf CASCADE;

-- create table
CREATE TABLE gmf.shakemap_acm7p2_lrdmf(
    site_id varchar,
    gmv_PGA float,
    "gmv_SA(0.1)" float,
    "gmv_SA(0.2)" float,
...
COPY gmf.shakemap_acm7p2_lrdmf (site_id,gmv_PGA,"gmv_SA(0.1)","gmv_SA(0.2)","gmv_SA(0.3)","gmv_SA(0.5)","gmv_SA(0.6)","gmv_SA(1.0)","gmv_SA(2.0)",lon,lat)
    FROM 'D:\workspace\data\source datasets\dsra\s_shakemap_ACM7p2_LRDMF_234.txt'
