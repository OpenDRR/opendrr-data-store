...
CREATE SCHEMA IF NOT EXISTS results_dsra_idm6p8_jdfpathways;
...
CAST(CAST(ROUND(CAST(COALESCE(AVG((a."sL_Str_r2" + a."sL_NStr_r2" + a."sL_Cont_r2")/(NULLIF((b.structural + b.nonstructural + b.contents),0))),0) AS NUMERIC),6) AS FLOAT) AS NUMERIC) AS "sLm_Asset_r2",
CAST(CAST(ROUND(CAST(SUM(a."sL_Str_r2" + a."sL_NStr_r2") AS NUMERIC),6) AS FLOAT) AS NUMERIC) AS "sLt_Bldg_r2",
CAST(CAST(ROUND(CAST(COALESCE(AVG((a."sL_Str_r2" + a."sL_NStr_r2")/(NULLIF((b.structural + b.nonstructural + b.contents),0))),0) AS NUMERIC),6) AS FLOAT) AS NUMERIC) AS "sLmr_Bldg_r2",
...
FROM dsra.dsra_idm6p8_jdfpathways a
...
LEFT JOIN gmf.shakemap_idm6p8_jdfpathways_metrovan_sauid_xref e ON b.sauid = e.sauid
