...
FOR %%x IN (nhsl_physical_exposure_indicators_hexgrid_1km, ^
...
nhsl_physical_exposure_indicators_hexgrid_100km_uc) DO ogr2ogr -f "gpkg" D:\Workspace\data\view_outputs\all_indicators\hexgrid\%%x.gpkg PG:"host=localhost user=postgres dbname=opendrr password=admin" -sql "SELECT * FROM results_nhsl_physical_exposure.%%x" -nln %%x
