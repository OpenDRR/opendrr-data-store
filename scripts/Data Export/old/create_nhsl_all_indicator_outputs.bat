...
FOR %%x IN (nhsl_physical_exposure_all_indicators_b) DO ogr2ogr -f "gpkg" D:\Workspace\data\view_outputs\merged_indicators\%%x.gpkg PG:"host=localhost user=postgres dbname=opendrr_allscenarios password=admin" results_nhsl_physical_exposure.%%x

REM Geopackage Shapefile export
REM FOR %%x IN (nhsl_physical_exposure_all_indicators_s) DO ogr2ogr -f "gpkg" D:\Workspace\data\view_outputs\merged_indicators\%%x.gpkg PG:"host=localhost user=postgres dbname=opendrr_allscenarios password=admin" results_nhsl_physical_exposure.%%x
